...
| FRZ-002 | Freeze approval | `host-device-control-poc-system` repository tree identified as `v0.2.2` | Upstream Framework/Template `v1.1.2` alignment and replaceable Presentation-boundary governance only; Protocol, wire behavior, transport policy, implementation gaps, and prior evidence conclusions unchanged | Approved promotion of reviewed `v0.2.2-rc.1` content to frozen `v0.2.2`; exact final Git commit/tag/Release identity and release approval remain pending | Repository owner | AI-assisted recorder | 2026-07-29 | Explicit user instruction to freeze; see `CHANGELOG.md`, `FRAMEWORK_REFERENCE.md`, and `baselines/repositories.yaml` |
